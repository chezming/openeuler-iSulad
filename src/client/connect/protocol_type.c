...
static void isula_images_history_free(size_t history_num, struct isula_history_info *history_list)
{
    size_t i = 0;
    ...
    for (i = 0; i < history_num; i++) {
        free(history_list[i].comment);
        free(history_list[i].create_by);
        free(history_list[i].id);
    }
