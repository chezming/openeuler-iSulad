...
static struct isulad_engine_operation_lists g_isulad_engines;
...
int engines_global_init()
{
    ...
    (void)memset(&g_isulad_engines, 0, sizeof(struct isulad_engine_operation_lists));
...
static struct engine_operation *query_engine(const char *name)
{
    ...
    if (g_isulad_engines.op != NULL && g_isulad_engines.op->engine_type != NULL && strcasecmp(name, g_isulad_engines.op->engine_type) == 0) {
