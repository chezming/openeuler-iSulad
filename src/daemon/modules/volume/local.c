...
static struct volume * new_volume(char *name, char *path)
{
...
static bool load_volume(const char *root_dir, const struct dirent *dir, void *userdata)
{
    ...
    vol = new_volume((char *)dir->d_name, data_dir);
    if (vol == NULL) {
        ERROR("out of memory");
        ret = -1;
        goto out;
...
static int do_volume_create(char *name, struct volume **vol)
{
...
struct volume * local_volume_create(char *name)
{
    ...
    if (v != NULL) {
        ...
        v = new_volume(v->name, v->path);
        mutex_unlock(&g_volumes->mutex);
        return v;
    }
    mutex_unlock(&g_volumes->mutex);

    ret = do_volume_create(name, &v);
    if (ret != 0) {
...
int local_volume_remove(char *name)
{
    ...
    path = util_strdup_s(v->path);

    if (remove_volume_dir(path) != 0) {
        ERROR("failed to remove volume dir %s: %s", path, strerror(errno));
        ret = -1;
        goto out;
    }

    if (map_search(g_volumes->vols_by_name, name) != NULL) {
