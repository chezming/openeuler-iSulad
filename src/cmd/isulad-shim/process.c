...
static int add_io_dispatch(int epfd, io_thread_t *io_thd, int from, int to)
{
    ...
    fd_node_t *fn = calloc(1, sizeof(fd_node_t));
    if (fn == NULL) {
        return SHIM_ERR;
    }
    fn->fd = to;
...
static int process_io_start(process_t *p, int std_id)
{
    ...
    io_thd = calloc(1, sizeof(io_thread_t));
...
#define RAND_NUM_LEN 9
    ...
    p->console_sock_path = calloc(1, MAX_CONSOLE_SOCK_LEN + 1);
...
static stdio_t* initialize_io(process_t *p)
{
    ...
    p->stdio = calloc(1, sizeof(stdio_t));
...
int create_process(process_t *p)
{
    ...
    if (pid == (pid_t)0) {
        ...
        char *pid_path = calloc(1, PATH_MAX);
        ...
        if (p->state->exec) {
            process_desc = calloc(1, PATH_MAX);
