...
function set_up()
{
	...
	CID=$(sudo isula run -itd ${image} sh)
...
function test_concurrent_bigdata_stream()
{
	...
	for index in $(seq 1 5); do
		isula exec -it $CID cat test_500M > /tmp/iocopy_stream_data_500M_$index &
	done
	wait
...
function test_more_concurrent_stream()
{
	...
	for index in $(seq 1 30); do
		isula exec -it $CID cat test_20M > /tmp/iocopy_stream_data_20M_$index &
	done
	wait
...
function test_stream_with_stop_client()
{
	...
	isula exec -it $CID cat test_500M > /tmp/iocopy_stream_data_500M &
	pid=$!
	sleep 2
	kill -19 $pid
	sleep 5
