 ...
 * Author: lijiaming
 * Create: 2023-09-07
 * Description: provide fuzz test for im_logout
 ******************************************************************************/
...
{
    if (size < sizeof(im_logout_request))
        return 0;
    im_logout_request *request = (im_logout_request *)malloc(sizeof(im_logout_request));
    ...
    int result = im_logout(request, &response);
    free(request);
    free(data_str);
    if (response == NULL)
        return 0;
